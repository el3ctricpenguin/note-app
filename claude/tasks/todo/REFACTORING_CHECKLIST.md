# コードリファクタリング実装チェックリスト

## 🎯 目標
- コード量30-40%削減 (現在: **2,289行** → 目標: **1,375-1,602行**)
- 可読性・保守性向上
- バグの減少

## 📊 現在のコード量 (リファクタリング前)

### 総計
- **TypeScript/TSXファイル**: 47ファイル
- **総行数**: 2,289行
- **設定ファイル**: 88行 (package.json, tsconfig.json, next.config.mjs)

### ディレクトリ別内訳
- **src/app/** (APIルート・ページ): 902行
  - API routes: 298行
  - Pages: 604行
- **src/components/**: 770行
  - Modals: 517行 (最大削減対象)
  - Cards: 207行
  - Layout: 73行
  - Form/Context: 45行
- **src/config/**: 189行
  - Theme設定: 184行 (簡素化可能)
- **src/lib/**: 53行
- **src/features/**: 15行
- **src/types/**: 5行 (拡張予定)
- **その他**: 355行

### 削減ターゲット
- **APIルート重複**: 298行 → 約200行 (33%削減)
- **モーダルコンポーネント**: 517行 → 約350行 (32%削減)  
- **テーマ設定**: 184行 → 約120行 (35%削減)

---

## 📋 高優先度タスク

### ✅ 1. 共通ユーティリティ作成

#### 1.1 API共通ユーティリティ (`src/lib/api-utils.ts`)
- [ ] エラーハンドリング関数 `handleApiError()`
- [ ] 成功レスポンス関数 `createSuccessResponse()`
- [ ] バリデーション関数 `validateRequest()`
- [ ] 認証チェック関数 `requireAuth()`

#### 1.2 共通バリデーションスキーマ (`src/lib/validation.ts`)
- [ ] 認証スキーマ統合 (sign-in/sign-up共通化)
- [ ] 映画関連スキーマ (watched/watchlist)
- [ ] TODOスキーマ
- [ ] 共通型定義

### ✅ 2. API重複除去

#### 2.1 認証API統合
- [ ] `src/app/api/(auth)/sign-in/route.ts` - 共通ユーティリティ適用
- [ ] `src/app/api/(auth)/sign-up/route.ts` - 共通ユーティリティ適用
- [ ] `src/app/api/(auth)/sign-out/route.ts` - 共通ユーティリティ適用

#### 2.2 映画API最適化
- [ ] `src/app/api/film/watched/route.ts` - 共通化
- [ ] `src/app/api/film/watched/[watchedFilmId]/route.ts` - 重複削除
- [ ] `src/app/api/film/watched/by-date/route.ts` - SQLクエリ最適化
- [ ] `src/app/api/film/watchlist/route.ts` - 共通化
- [ ] `src/app/api/film/watchlist/[watchlistId]/route.ts` - 重複削除

#### 2.3 TODO API最適化
- [ ] `src/app/api/todo/route.ts` - 共通化
- [ ] `src/app/api/todo/[id]/route.ts` - 2回クエリを1回に削減

### ✅ 3. データベース操作最適化

#### 3.1 クエリ効率化
- [ ] TODO toggle: findUnique + update → updateとfindUniqueの統合
- [ ] 映画の日付グループ化: メモリ処理 → SQL GROUP BY
- [ ] 不要なNumber()変換の削除

#### 3.2 エラーハンドリング統一
- [ ] Prismaエラーの統一的処理
- [ ] データベース接続エラーの処理
- [ ] トランザクション処理の追加（必要に応じて）

---

## 📋 中優先度タスク

### ✅ 4. 設定ファイル整備

#### 4.1 開発ツール設定
- [ ] `.prettierrc` 作成・設定
- [ ] `.eslintrc.json` 設定強化
- [ ] `tsconfig.json` 最適化
- [ ] `next.config.mjs` 設定追加

#### 4.2 型定義強化
- [ ] `src/types/index.ts` - API型定義追加
- [ ] Prismaクライアント型の再利用
- [ ] 共通インターフェースの定義

### ✅ 5. コンポーネント最適化

#### 5.1 レイアウト・UI
- [ ] `src/components/layout/` - 不要なpropsの削除
- [ ] `src/components/modals/` - 共通モーダルロジック抽出
- [ ] `src/components/cards/` - propsの型安全性向上

#### 5.2 テーマ・スタイル
- [ ] `src/config/theme/` - 未使用スタイルの削除
- [ ] Chakra UIテーマ設定の簡素化
- [ ] CSS-in-JS最適化

---

## 📋 低優先度タスク

### ✅ 6. パフォーマンス改善

#### 6.1 コード分割
- [ ] 動的インポートの追加
- [ ] バンドルサイズ分析
- [ ] 不要な依存関係の削除

#### 6.2 セキュリティ強化
- [ ] 入力サニタイゼーション
- [ ] レート制限の実装
- [ ] CORS設定の見直し

### ✅ 7. 開発体験向上

#### 7.1 開発ツール
- [ ] ホットリロード最適化
- [ ] デバッグ設定の改善
- [ ] ログ出力の統一

#### 7.2 ドキュメント
- [ ] API仕様書の作成
- [ ] コンポーネントドキュメント
- [ ] 開発ガイドライン更新

---

## 🧪 テスト・検証

### ✅ 実装後の確認項目

#### 機能テスト
- [ ] 認証フロー（サインイン・サインアップ・サインアウト）
- [ ] 映画管理（追加・編集・削除・一覧表示）
- [ ] TODO管理（追加・完了切替・削除）
- [ ] 日付別映画一覧の表示

#### 技術テスト
- [ ] `npm run build` エラーなし
- [ ] `npm run lint` エラーなし
- [ ] TypeScript型チェック通過
- [ ] データベースマイグレーション確認

#### パフォーマンステスト
- [ ] ページ読み込み速度
- [ ] API レスポンス時間
- [ ] バンドルサイズ削減確認

---

## 📊 進捗管理

### 完了済み
- [x] 現状分析・問題点特定
- [x] リファクタリング計画策定

### 実装中
- [ ] 高優先度タスク (0/3完了)
- [ ] 中優先度タスク (0/2完了)
- [ ] 低優先度タスク (0/2完了)

### 推定作業時間
- **高優先度**: 8-12時間
- **中優先度**: 4-6時間  
- **低優先度**: 2-4時間
- **合計**: 14-22時間

### 期待される削減量
- **コード行数**: 687-914行削減 (30-40%)
- **API重複**: 70%削減 (約209行削除)
- **エラーハンドリング**: 90%共通化
- **型安全性**: 95%向上

### リファクタリング後の目標行数
- **最小目標**: 1,375行 (40%削減)
- **現実的目標**: 1,602行 (30%削減)

---

## 🚀 実装順序の推奨

1. **第1段階**: 共通ユーティリティ作成 (1.1, 1.2)
2. **第2段階**: 認証API統合 (2.1)
3. **第3段階**: 映画API最適化 (2.2)
4. **第4段階**: TODO API最適化 (2.3)
5. **第5段階**: データベース最適化 (3.1, 3.2)
6. **第6段階**: 設定・型定義整備 (4.1, 4.2)
7. **第7段階**: UI・パフォーマンス改善 (5.1, 5.2, 6.1)

**注意**: 各段階完了後は必ずテストを実行し、動作確認を行うこと